		...
		t.Run(tc.name, func(t *testing.T) {
			output := captureOutput(t, func() {
				...
				synctest.Test(t, func(t *testing.T) {
					if err := WorkerPool(context.Background(), tc.workers, tc.nums); err != nil {
						t.Fatal(err)
					}
				})
			})
...
func TestWorkerPoolInvalidWorkerCount(t *testing.T) {
	for _, workers := range []int{-1, 0} {
		t.Run(fmt.Sprint(workers), func(t *testing.T) {
			output := captureOutput(t, func() {
				synctest.Test(t, func(t *testing.T) {
					if err := WorkerPool(context.Background(), workers, []int{1, 2}); err == nil {
		...
		t.Run(scenario, func(t *testing.T) {
			output := captureOutput(t, func() {
				synctest.Test(t, func(t *testing.T) {
					...
					err := WorkerPool(ctx, 3, []int{1, 2, 3, 4})
					if !errors.Is(err, want) {
						t.Fatalf("WorkerPool = %v; want %v", err, want)
					}
				})
			})
			// Частичную сумму нельзя выдавать за полный результат.
			if output != "" {
				t.Fatalf("cancelled pool produced output: %q", output)
			}
		})
	}
}
